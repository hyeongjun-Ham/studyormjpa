...
public class HttpRequestHandlerV3 implements Runnable {
    ...
    private void process() throws IOException {
        try (socket;
            ...
            String requesstString = requestToString(reader);
            if (requesstString.contains("/favicon.ico")) {
                log("favicon 요청");
                return;
            }
            log("HTTP 요청 정보 출력");
            System.out.println(requesstString);

            log("HTTP 응답 생성중...");
            if (requesstString.startsWith("GET /site1")) {
                site1(writer);
            } else if (requesstString.startsWith("GET /site2")) {
                site2(writer);
            } else if (requesstString.startsWith("GET /search")) {
                search(writer, requesstString);
            } else if (requesstString.startsWith("GET / ")) {
                home(writer);
            } else {
                notFound(writer);
            }
    ...
    private void search(PrintWriter writer, String requesstString) {
        int startIndex = requesstString.indexOf("q=");
        int endIndex = requesstString.indexOf(" ", startIndex + 2);
        String query = requesstString.substring(startIndex + 2, endIndex);
